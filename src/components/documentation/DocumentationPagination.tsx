...
export function DocumentationPagination({ currentPage }: DocumentationPaginationProps) {
  const currentIndex = documentationPages.findIndex((page) => page.id === currentPage);
  const prevPage = currentIndex > 0 ? documentationPages[currentIndex - 1] : null;
  const nextPage = currentIndex < documentationPages.length - 1 ? documentationPages[currentIndex + 1] : null;
  const navigate = useNavigate();
  
  const currentPageDesc = pageDescriptions[currentPage as keyof typeof pageDescriptions] || {};
        ...
        {nextPage && (
          <PaginationItem className="ml-auto">
            ...
            >
              <PaginationNext
                href="javascript:void(0)"
                className="flex flex-col items-end gap-1 border border-border bg-[#20252e] text-white hover:bg-[#2A324A]"
              >
